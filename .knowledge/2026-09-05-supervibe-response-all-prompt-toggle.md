...
> 과거 설계 기록: 2026-09-05 당시 버전·작업 순서·검수 조건을 보존한다. 아래 계획을 현재 미완료 작업이나 새 실행 승인으로 해석하지 않는다. 현재 동작은 [src/state.rs](../src/state.rs)와 [src/renderer.rs](../src/renderer.rs)의 응답 묶음·접기 처리로 확인하고, 배포 여부는 [배포 기록](배포-버전-갱신.md)과 변경 이력을 대조한다.
