...
- `session/fork`는 살아 있는 부모 세션의 `model`과 `effort`를 우선한다. 호출자가 넘긴 값은 부모가 브리지에 없을 때만 사용한다. 부모가 턴 사이에 모델이나 추론 수준을 바꾼 뒤 호출자의 시작 기본값이 남아 있어도 분기 세션은 부모의 현재 선택을 이어야 한다.

## 확인 기록

### 2026-09-10 사용량 한도 자동 재개 — 1.8.22 배포

- 대화형 CLI의 자동 재개는 SDK 실행에는 적용되지 않아 브리지에서 처리한다. 설치된 SDK 0.3.267의 `SDKRateLimitInfo`와 `SDKAssistantMessageError`를 대조했다. 현재 턴의 주 에이전트가 `rate_limit` 오류로 실패하고, 구독 한도 `rejected`와 앞으로 24시간 이내인 `resetsAt`이 함께 확인될 때만 대기한다. 일반 API 오류·사용량 경고·추가 과금 사용 중·초기화 시각 누락에는 예약하지 않는다.
- 기존 query와 턴을 유지하고 초기화 시각 1초 뒤 고정된 이어가기 메시지를 한 번 넣는다. 원래 요청·이미지를 재전송하지 않으며 모델·추론 강도·역할 도구 제한·권한 확인을 그대로 사용한다. 반복 한도는 최대 세 번까지 자동 재개한다.
- 화면에 대기 시간을 표시한다. Esc·Ctrl+C·새 요청·세션 전환·연결 종료가 예약을 취소한다. 30분 넘는 절전 중 초기화되면 직접 요청을 보내야 한다. 프로세스 재시작 시 예약은 복원하지 않는다. Codex에는 적용하지 않는다.
- 검증: 브리지 자체 검사에서 SDK 이벤트 재생·실제 1초 타이머·중복 콜백·취소·새 요청·세션 종료·잘못된 초기화 시각·일반 오류·성공 결과·반복 제한·장기 절전을 확인했다. Rust 전체 검사 1,294개 통과·14개 기본 제외이며, 대기 표시와 Esc 취소 상태 검사를 포함한다. `git diff --check`도 통과했다. 첫 전체 검사 빌드는 임시 개체 파일 잠금으로 실패했고 재실행은 통과했다. 실제 계정 한도 소진·초기화 및 실제 터미널 화면 검증은 하지 않았다. 사용자 승인 후 1.8.22로 공개 배포하고 패키지·설치본 해시와 관리 실행 버전을 확인했다.
...
| --- | --- | --- | --- |
| 2026-09-23 | 0.3.280 / 2.1.280 | 버전 상향·신모델 반영 | 0.3.276 → 0.3.280. `sdk.d.ts` 차이는 `readMcpResource`·`thinkingDisplay: highlights`·`updateSettings(userSettings)` 추가와 사용량 행 `severity`·`is_active` 필수화뿐이고 브리지 사용처는 없다. 실제 `model/list`에서 `opus[1m]`이 `Opus 5.5`(기본)로 확인돼 예비 목록 표시명과 단가($4/$20, 캐시 읽기 0.05배)를 반영했다. `claude-opus-5-5`는 `claude-opus-5` 접두 일치보다 먼저 두어야 한다. 사용자 지시로 Opus 4.8을 목록에서 제거했다. 순정 CLI의 차단 대체 모델은 `opus5`(번들 내 `XS="opus5"`, `getRefusalFallbackOpusModel`)라서 브리지가 `claude-opus-5` 행을 `hidden: true`로 합성하고, 호스트는 이를 `/model`에서 숨긴 채 차단 전환 대상으로만 쓴다. 실제 대화 턴 검증은 하지 않았다. |
| 2026-09-18 | 0.3.276 | 버전 상향·기능 반영 | 0.3.273 → 0.3.276. `sdk.d.ts` 차이는 추가뿐이고 제거된 `Query` 메서드·필드는 없다. 결과의 `startup_failure_reason`을 사유별 한국어 안내로 바꿔 오류 앞줄에 붙이고, 자체 검사에서 설치 SDK의 사유 목록과 대조해 누락을 잡는다. `McpServerStatus.source`는 MCP 상세의 출처 줄로 표시한다. `projectConfigRoot`는 워크트리 설정 루트 지정용이라 현재 쓰지 않는다. |
