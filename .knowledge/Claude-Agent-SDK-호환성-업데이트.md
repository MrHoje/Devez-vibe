...
- 검증: 브리지 자체 검사에서 SDK 이벤트 재생·실제 1초 타이머·중복 콜백·취소·새 요청·세션 종료·잘못된 초기화 시각·일반 오류·성공 결과·반복 제한·장기 절전을 확인했다. Rust 전체 검사 1,294개 통과·14개 기본 제외이며, 대기 표시와 Esc 취소 상태 검사를 포함한다. `git diff --check`도 통과했다. 첫 전체 검사 빌드는 임시 개체 파일 잠금으로 실패했고 재실행은 통과했다. 실제 계정 한도 소진·초기화 및 실제 터미널 화면 검증과 공개 배포는 하지 않았다.
