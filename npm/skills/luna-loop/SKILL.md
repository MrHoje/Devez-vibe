...
- 검증자 명령은 백그라운드 실행으로 띄운다. 프로세스가 끝나거나 죽으면 종료 코드가 담긴 완료 알림이 오므로, 알림이 아직 없다는 사실만으로 완료나 실패를 단정하지 않는다.
- 검증자마다 로그 파일을 `Monitor`로 감시해 종료 이벤트와 정지를 알림으로 받는다. 아래 스크립트는 최상위 종료 이벤트가 나타나면 그 유형을 출력하고 끝나며, 5분 넘게 로그가 자라지 않으면 `STALLED`를 출력하고 끝난다. 감시 자체가 먼저 끊기지 않도록 `timeout_ms`는 검증 예상 시간보다 길게(예: 3600000) 준다.

  ```bash
  log=<로그 파일>
  while true; do
    if grep -q -E '^\{"type":"(turn\.completed|turn\.failed|error)"' "$log" 2>/dev/null; then
      grep -o -E '^\{"type":"(turn\.completed|turn\.failed|error)"' "$log" | cut -d'"' -f4 | sort -u
      exit 0
    fi
    if [ -e "$log" ] && [ -z "$(find "$log" -mmin -5)" ]; then
      echo "STALLED: 5분 넘게 로그 갱신 없음"
      exit 0
    fi
    sleep 10
  done
...
- `STALLED` 알림을 받으면 검증자 프로세스가 살아 있는지와 결과 파일 유무를 확인하고, 그래도 로그가 자라지 않으면 해당 백그라운드 작업을 `TaskStop`으로 끝낸 뒤 `시간 초과`로 기록하고 아래 재시도 규칙을 적용한다. 정지한 검증자를 기다리는 동안 다른 검증자의 결과만으로 전체를 완료로 판정하지 않는다.
